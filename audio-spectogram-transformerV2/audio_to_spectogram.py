...
FMIN          = 100              # Hz  – lower bound of extracted range
...
def remove_silent_frames(y: np.ndarray, sr: int,
    ...
    return kept_frames.ravel(order="F")
...
def process_file(mp3_path: Path, out_path: Path,
    ...
    try:
        ...
        # Skip if too little signal remains after silence removal
        MIN_SAMPLES = int(SR * 1.0)   # at least 1 second of real signal
        if len(y) < MIN_SAMPLES:
            return rel, False, f"Too short after silence removal ({len(y)} samples)"

        segment_samples = segment_sec * sr
        saved           = 0
        starts          = range(0, max(1, len(y)), segment_samples)

        for i, start in enumerate(starts):
            segment = y[start: start + segment_samples]

            # ── FIX: skip instead of zero-pad ────────────────────────────
            # Require at least 1 second of real content in the segment.
            # The spectrogram_to_image function resizes whatever we give it
            # to 224×224, so shorter segments still produce full images.
            if len(segment) < MIN_SAMPLES:
                continue
            # No np.pad — use segment as-is
